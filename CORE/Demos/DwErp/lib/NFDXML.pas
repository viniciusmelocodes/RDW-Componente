...
function StrToInt64Def(const AValue: AnsiString; ADefault: int64): int64;
function StrToInt64(const AValue: AnsiString): int64;
{$ENDIF}
...
type
  ...
  // This event is used in the TNFDXML.OnNodeCompare event, and should
  ...
  // The TXmlNode represents an element in the XML file. Each TNFDXML  holds
  ...
  private
    ...
    FDocument: TNFDXML;         // Pointer to parent XmlDocument
    FElementType: TXmlElementType; // The type of element
    FName: AnsiString;                 // The element name
    ...
    FValue: AnsiString;                // The *escaped* value
    ...
    procedure SetValueAsString(const AValue: AnsiString);
    function GetIndent: AnsiString;
    function GetLineFeed: AnsiString;
    function GetTreeDepth: integer;
    function GetAttributeCount: integer;
    function GetAttributePair(Index: integer): AnsiString;
    ...
    function GetCascadedName: AnsiString;
    ...
    function GetBinaryString: AnsiString;
    procedure SetBinaryString(const Value: AnsiString);
    ...
    function GetAttributeByName(const AName: AnsiString): AnsiString;
    procedure SetAttributeByName(const AName, Value: AnsiString);
    ...
    procedure SetAttributeValueAsInteger(Index: integer;
      const Value: integer);
    function GetAttributeByNameWide(const AName: AnsiString): widestring;
    procedure SetAttributeByNameWide(const AName: AnsiString;
      const Value: widestring);
    function GetTotalNodeCount: integer;
    function FloatSignificantDigits: integer;
    function FloatAllowScientific: boolean;
    function GetAttributeValueDirect(Index: integer): AnsiString;
    procedure SetAttributeValueDirect(Index: integer; const Value: AnsiString);
  protected
    function CompareNodeName(const NodeName: AnsiString): integer;
    procedure DeleteEmptyAttributes;
    function GetNodes(Index: integer): TXmlNode; virtual;
    function GetNodeCount: integer; virtual;
    procedure ParseTag(const AValue: AnsiString; TagStart, TagClose: integer);
    procedure ReadFromStream(S: TStream); virtual;
    procedure ReadFromString(const AValue: AnsiString); virtual;
    procedure ResolveEntityReferences;
    function UnescapeString(const AValue: AnsiString): AnsiString; virtual;
    function Utf8Encoded: boolean;
    function WriteInnerTag: AnsiString; virtual;
    procedure WriteToStream(S: TStream); virtual;
    procedure ChangeDocument(ADocument: TNFDXML);
  public
    // Create a new TXmlNode object. ADocument must be the TNFDXML that is
    // going to hold this new node.
    constructor Create(ADocument: TNFDXML); virtual;
    // \Create a new TXmlNode with name AName. ADocument must be the TNFDXML
    // that is going to hold this new node.
    constructor CreateName(ADocument: TNFDXML; const AName: AnsiString); virtual;
    // \Create a new TXmlNode with name AName and string value AValue. ADocument
    // must be the TNFDXML that is going to hold this new node.
    constructor CreateNameValue(ADocument: TNFDXML; const AName, AValue: AnsiString); virtual;
    // \Create a new TXmlNode with XML element type AType. ADocument must be the

    constructor CreateType(ADocument: TNFDXML; AType: TXmlElementType); virtual;
    ...
    // node. You can also Assign a TNFDXML  document to the node, in that case
    // the RootNodeList property of the TNFDXML  object will be copied.
    ...
    // recursively freed when TNFDXML .Free is called.
    destructor Destroy; override;
    {$IFDEF D4UP}
    // Use this method to add an integer attribute to the node.
    procedure AttributeAdd(const AName: AnsiString; AValue: integer); overload;
    {$ENDIF}
    // Use this method to add a string attribute with value AValue to the node.
    procedure AttributeAdd(const AName, AValue: AnsiString); {$IFDEF D4UP}overload;{$ENDIF}
    ...
    function AttributeIndexByname(const AName: AnsiString): integer;
    ...
    function FindNode(const NodeName: AnsiString): TXmlNode;
    ...
    // items to TXmlNode.
    procedure FindNodes(const NodeName: AnsiString; const AList: TList);
    // Use FromAnsiString to convert a normal ANSI string to a string for the node
    // (name, value, attributes). If the TNFDXML  property UtfEncoded is True,
    ...
    function FromAnsiString(const s: AnsiString): AnsiString;
    // Use FromWidestring to convert widestring to a string for the node (name, value,
    // attributes). If the TNFDXML  property UtfEncoded is True, all
    // character codes higher than $FF are preserved.
    function FromWidestring(const W: widestring): AnsiString;
    // Use HasAttribute to determine if the node has an attribute with name AName.
    function HasAttribute(const AName: AnsiString): boolean; virtual;
    ...
    function NodeByAttributeValue(const NodeName, AttribName, AttribValue: AnsiString;
    ...
    function NodeByName(const AName: AnsiString): TXmlNode; virtual;
    ...
    function NodeFindOrCreate(const AName: AnsiString): TXmlNode; virtual;
    // Find the index of the first subnode with name AName.
    function NodeIndexByName(const AName: AnsiString): integer; virtual;
    ...
    function NodeIndexByNameFrom(const AName: AnsiString; AFrom: integer): integer; virtual;
    ...
    // pointer to it.
    function NodeNew(const AName: AnsiString): TXmlNode; virtual;
    // \Create a new node with AName, and insert it into the subnode list at location
    // Index, and return a pointer to it.
    function NodeNewAtIndex(Index: integer; const AName: AnsiString): TXmlNode; virtual;
    ...
    procedure NodesByName(const AName: AnsiString; const AList: TList);
    ...
    // be returned.
    function ReadAttributeBool(const AName: AnsiString; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}): boolean; virtual;
    function ReadAttributeDateTime(const AName: AnsiString; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}): TDateTime; virtual;
    // Find the attribute with AName, and convert its value to an integer. If the
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeInteger(const AName: AnsiString; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}): integer; virtual;
    // Find the attribute with AName, and convert its value to an int64. If the
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeInt64(const AName: AnsiString; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}): int64; virtual;
    // Find the attribute with AName, and convert its value to a float. If the
    // attribute is not found, or cannot be converted, the default ADefault will
    // be returned.
    function ReadAttributeFloat(const AName: AnsiString; ADefault: double {$IFDEF D4UP}= 0{$ENDIF}): double;
    function ReadAttributeString(const AName: AnsiString; const ADefault: AnsiString {$IFDEF D4UP}= ''{$ENDIF}): AnsiString; virtual;
    ...
    // be returned.
    function ReadBool(const AName: AnsiString; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}): boolean; virtual;
    {$IFDEF USEGRAPHICS}
    // Read the properties Color and Style for the TBrush object ABrush from the
    // subnode with AName.
    procedure ReadBrush(const AName: AnsiString; ABrush: TBrush); virtual;
    ...
    function ReadColor(const AName: AnsiString; ADefault: TColor {$IFDEF D4UP}= clBlack{$ENDIF}): TColor; virtual;
    // Read the properties \Name, Color, Size and Style for the TFont object AFont
    // from the subnode with AName.
    procedure ReadFont(const AName: AnsiString; AFont: TFont); virtual;
    // Read the properties Color, Mode, Style and Width for the TPen object APen
    // from the subnode with AName.
    procedure ReadPen(const AName: AnsiString; APen: TPen); virtual;
    {$ENDIF}
    // Read the subnode with AName and convert its value to TDateTime. If the
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadDateTime(const AName: AnsiString; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}): TDateTime; virtual;
    // Read the subnode with AName and convert its value to a double. If the
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadFloat(const AName: AnsiString; ADefault: double {$IFDEF D4UP}= 0.0{$ENDIF}): double; virtual;
    // Read the subnode with AName and convert its value to an int64. If the
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadInt64(const AName: AnsiString; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}): int64; virtual;
    // Read the subnode with AName and convert its value to an integer. If the
    // subnode is not found, or cannot be converted, ADefault will be returned.
    function ReadInteger(const AName: AnsiString; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}): integer; virtual;
    // Read the subnode with AName and return its string value. If the subnode is
    // not found, ADefault will be returned.
    function ReadString(const AName: AnsiString; const ADefault: AnsiString {$IFDEF D4UP}= ''{$ENDIF}): AnsiString; virtual;
    // Read the subnode with AName and return its widestring value. If the subnode is
    // not found, ADefault will be returned.
    function ReadWidestring(const AName: AnsiString; const ADefault: widestring {$IFDEF D4UP}= ''{$ENDIF}): widestring; virtual;
    ...
    // to a normal ANSI string. If the TNFDXML  property UtfEncoded is True,
    ...
    function ToAnsiString(const s: AnsiString): AnsiString;
    // Use ToWidestring to convert any string from the node (name, value, attributes)
    // to a widestring. If the TNFDXML  property UtfEncoded is True, all
    // character codes higher than $FF are preserved.
    function ToWidestring(const s: AnsiString): widestring;
    ...
    procedure WriteAttributeBool(const AName: AnsiString; AValue: boolean; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}); virtual;
    procedure WriteAttributeDateTime(const AName:AnsiString; AValue: TDateTime; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    ...
    // WriteOnDefault = False, no attribute will be added.
    procedure WriteAttributeInteger(const AName: AnsiString; AValue: integer; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    procedure WriteAttributeInt64(const AName: AnsiString; const AValue: int64; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    procedure WriteAttributeFloat(const AName: AnsiString; AValue: double; ADefault: double {$IFDEF D4UP} = 0{$ENDIF}); virtual;
    ...
    // will be added.
    procedure WriteAttributeString(const AName: AnsiString; const AValue: AnsiString; const ADefault: AnsiString {$IFDEF D4UP}= ''{$ENDIF}); virtual;
    ...
    procedure WriteBool(const AName: AnsiString; AValue: boolean; ADefault: boolean {$IFDEF D4UP}= False{$ENDIF}); virtual;
    {$IFDEF USEGRAPHICS}
    ...
    procedure WriteBrush(const AName: AnsiString; ABrush: TBrush); virtual;
    // Add or replace the subnode with AName and set its value to represent the TColor
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteColor(const AName: AnsiString; AValue: TColor; ADefault: TColor {$IFDEF D4UP}= clBlack{$ENDIF}); virtual;
    ...
    procedure WriteFont(const AName: AnsiString; AFont: TFont); virtual;
    // Write properties Color, Mode, Style and Width of the TPen object APen to
    // the subnode with AName. If AName does not exist, it will be created.
    procedure WritePen(const AName: AnsiString; APen: TPen); virtual;
    {$ENDIF}
    ...
    procedure WriteDateTime(const AName: AnsiString; AValue: TDateTime; ADefault: TDateTime {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the double
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteFloat(const AName: AnsiString; AValue: double; ADefault: double {$IFDEF D4UP}= 0.0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the hexadecimal representation of
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteHex(const AName: AnsiString; AValue: integer; Digits: integer; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the int64
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteInt64(const AName: AnsiString; AValue: int64; ADefault: int64 {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the integer
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteInteger(const AName: AnsiString; AValue: integer; ADefault: integer {$IFDEF D4UP}= 0{$ENDIF}); virtual;
    // Add or replace the subnode with AName and set its value to represent the string
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteString(const AName, AValue: AnsiString; const ADefault: AnsiString {$IFDEF D4UP}= ''{$ENDIF}); virtual;
    ...
    function WriteToString: AnsiString; virtual;
    // Add or replace the subnode with AName and set its value to represent the widestring
    // AValue. If AValue = ADefault, and WriteOnDefault = False, no subnode will be added.
    procedure WriteWidestring(const AName: AnsiString; const AValue: widestring; const ADefault: widestring {$IFDEF D4UP}= ''{$ENDIF}); virtual;
    ...
    // existing one.
    property AttributeByName[const AName: AnsiString]: AnsiString read GetAttributeByName write
      SetAttributeByName;
    ...
    // existing one.
    property AttributeByNameWide[const AName: AnsiString]: widestring read GetAttributeByNameWide write
    ...
    property AttributeName[Index: integer]: AnsiString read GetAttributeName write SetAttributeName;
    ...
    property AttributePair[Index: integer]: AnsiString read GetAttributePair;
    // Read this property to get the string value of the attribute at index Index.
    // Write to it to set the string value.
    property AttributeValue[Index: integer]: AnsiString read GetAttributeValue write SetAttributeValue;
    ...
    // XML will be invalid.
    property AttributeValueDirect[Index: integer]: AnsiString read GetAttributeValueDirect write SetAttributeValueDirect;
    ...
    property BinaryString: AnsiString read GetBinaryString write SetBinaryString;
    ...
    property CascadedName: AnsiString read GetCascadedName;
    // Pointer to parent NativeXml document, or Nil if none.
    property Document: TNFDXML read FDocument write FDocument;
    ...
    property FullPath: AnsiString read GetFullPath;
    ...
    // separate them.
    property ValueDirect: AnsiString read FValue write FValue;
  ...
  public
    ...
    function ByAttribute(const AName, AValue: AnsiString): TXmlNode;
    property Items[Index: Integer]: TXmlNode read GetItems write SetItems; default;
  end;

  // TNFDXML  is the XML document holder. Create a TNFDXML  and then use
  ...
  private
    ...
    FIndentString: AnsiString;          // The indent string used to indent content (default is two spaces)
    ...
    function GetCommentString: AnsiString;
    procedure SetCommentString(const Value: AnsiString);
    function GetEntityByName(AName: AnsiString): AnsiString;
    function GetRoot: TXmlNode;
    function GetEncodingString: AnsiString;
    procedure SetEncodingString(const Value: AnsiString);
    function GetVersionString: AnsiString;
    procedure SetVersionString(const Value: AnsiString);
    function GetStyleSheetNode: TXmlNode;
  protected
    procedure CopyFrom(Source: TNFDXML); virtual;
    procedure DoProgress(Size: integer);
    function LineFeed: AnsiString; virtual;
  ...
  public
    ...
    constructor CreateName(const ARootName: AnsiString); virtual;
    // Destroy will free all data in the TNFDXML  object. This includes the
    // root node and all subnodes under it. Do not call Destroy directly, call
    // Free instead.
    destructor Destroy; override;
    // When calling Assign with a Source object that is a TNFDXML , will cause
    ...
    // begin
    //   if (Node.Name = 'LastNode') and (Sender is TNFDXML ) then
    //     TNFDXML (Sender).AbortParsing := True;
    // end;
    // </code>
    ...
    property CommentString: AnsiString read GetCommentString write SetCommentString;
    ...
    property EncodingString: AnsiString read GetEncodingString write SetEncodingString;
    ...
    property EntityByName[AName: AnsiString]: AnsiString read GetEntityByName;
    ...
    property IndentString: AnsiString read FIndentString write FIndentString;
    ...
    property VersionString: AnsiString read GetVersionString write SetVersionString;
...
{$ENDIF}
  ...
  private
    FBuffer: AnsiString;                // Buffer that holds temporary utf8 characters
  ...
  public
    ...
    property Surplus: AnsiString read FSurplus write FSurplus;
  ...
  public
    ...
    procedure AddString(var S: AnsiString);
    function StringCopy(AFirst, ALength: integer): AnsiString;
...
function EscapeString(const AValue: AnsiString): AnsiString;

// Replace all escaped characters in string AValue by their original. This includes
// character references using &#...; and &#x...;
function UnEscapeStringUTF8(const AValue: AnsiString): AnsiString;

// Replace all escaped characters in string AValue by their original. This includes
// character references using &#...; and &#x...;, however, character codes above
// 255 are not replaced.
function UnEscapeStringANSI(const AValue: AnsiString): AnsiString;

// Enclose the string AValue in quotes.
function QuoteString(const AValue: AnsiString): AnsiString;
// Remove the quotes from string AValue.
function UnQuoteString(const AValue: AnsiString): AnsiString;

// This function adds control characters Chars repeatedly after each Interval
// of characters to string Value.
function AddControlChars(const AValue: AnsiString; const Chars: AnsiString; Interval: integer): AnsiString;

// This function removes control characters from string AValue (Tab, CR, LF and Space)
function RemoveControlChars(const AValue: AnsiString): AnsiString;
...
function sdDateTimeFromString(const ADate: AnsiString): TDateTime;

// Convert the string ADate to a TDateTime according to the W3C date/time specification
// as found here: http://www.w3.org/TR/NOTE-datetime
// If there is a conversion error, the default value ADefault is returned.
function sdDateTimeFromStringDefault(const ADate: AnsiString; ADefault: TDateTime): TDateTime;

// Convert the TDateTime ADate to a string according to the W3C date/time specification
// as found here: http://www.w3.org/TR/NOTE-datetime
function sdDateTimeToString(ADate: TDateTime): AnsiString;
...
function sdWriteNumber(Value: double; SignificantDigits: integer; AllowScientific: boolean): AnsiString;

// Conversion between Ansi, UTF8 and Unicode

// Convert a widestring to a UTF8 encoded string
function sdUnicodeToUtf8(const W: widestring): AnsiString;

// Convert a normal ansi string to a UTF8 encoded string
function sdAnsiToUtf8(const S: AnsiString): AnsiString;

// Convert a UTF8 encoded string to a widestring
function sdUtf8ToUnicode(const S: AnsiString): widestring;

// Convert a UTF8 encoded string to a normal ansi string
function sdUtf8ToAnsi(const S: AnsiString): AnsiString;
...
function FindString(const SubString: AnsiString; const S: AnsiString; Start, Close: integer; var APos: integer): boolean;
...
function MatchString(const SubString: AnsiString; const S: AnsiString; Start: integer): boolean;
...
procedure ParseAttributes(const AValue: AnsiString; Start, Close: integer; Attributes: TStrings);
...
function TrimPos(const AValue: AnsiString; var Start, Close: integer): boolean;

// Encoding/Decoding functions

// Encode binary data in Source as BASE64. The function returns the BASE64 encoded
// data as string, without any linebreaks.
function EncodeBase64(const Source: AnsiString): AnsiString;
...
function DecodeBase64(const Source: AnsiString): AnsiString;

// Encode binary data in Source as BINHEX. The function returns the BINHEX encoded
// data as string, without any linebreaks.
function EncodeBinHex(const Source: AnsiString): AnsiString;

// Decode BINHEX data in Source into binary data. The function returns the binary
// data as string. Use a TStringStream to convert this data to a stream. The Source
// string may contain linebreaks and control characters, these will be stripped.
function DecodeBinHex(const Source: AnsiString): AnsiString;
...
constructor TsdStringStream.Create(const S: AnsiString);
begin
  inherited Create;
  SetSize(length(S));
  if Size > 0 then
...
function TsdStringStream.DataString: AnsiString;
begin
...
function UnEscapeStringUTF8(const AValue: AnsiString): AnsiString;
var
  SearchStr, Reference, Replace: AnsiString;
...
{$ELSE}
{$IFDEF VER230}
  Result := AnsiQuotedStr(string(AValue), Char(AQuoteChar));
{$ELSE}
...
procedure WriteStringToStream(S: TStream; const AString: AnsiString);
begin
...
function ReadStringFromStreamUntil(AReader: TsdSurplusReader; const ASearch: AnsiString;
  var AValue: AnsiString; SkipQuotes: boolean): boolean;
var
...
function ReadStringFromStreamWithQuotes(S: TStream; const Terminator: AnsiString;
...
function EncodeBase64Buf(const Buffer: TBytes; Count: Integer): AnsiString;
begin
...
procedure DecodeBase64Buf(const Source: AnsiString; var Buffer: TBytes; Count: Integer);
var
  ADecoded: TBytes;
begin
...
function PtrUnicodeToUtf8(Dest: PAnsiChar; MaxDestBytes: Cardinal; Source: PWideChar; SourceChars: Cardinal): Cardinal;
var
  i, count: Cardinal;
  c: Cardinal;
begin
...
function PtrUtf8ToUnicode(Dest: PWideChar; MaxDestChars: Cardinal; Source: PAnsiChar;
...
begin
  ...
  L := PtrUnicodeToUtf8(PAnsiChar(Temp), Length(Temp) + 1, PWideChar(W), Length(W));
  if L > 0 then
    SetLength(Temp, L - 1)
  else
    Temp := '';
...
begin
  ...
  L := PtrUtf8ToUnicode(PWideChar(Temp), Length(Temp)+1, PAnsiChar(S), Length(S));
  if L > 0 then
    SetLength(Temp, L-1)
  else
    Temp := '';
...
function EncodeBase64Buf(const Buffer; Count: Integer): AnsiString;
var
  i, j: integer;
  ACore: integer;
  ALong: cardinal;
...
procedure DecodeBase64Buf(var Source: AnsiString; var Buffer; Count: Integer);
var
  i, j: integer;
  BufPos, Core: integer;
  LongVal: cardinal;
...
function EncodeBinHexBuf(const Source; Count: Integer): AnsiString;
// Encode binary data in Source as BINHEX. The function returns the BINHEX encoded
// data as string, without any linebreaks.
var
{$IFDEF CLR}
...
procedure DecodeBinHexBuf(const Source: AnsiString; var Buffer{$IFDEF CLR}: TBytes{$ENDIF}; Count: Integer);
...
{$ELSE}
{$IFDEF D4UP}
  HexToBin(PAnsiChar(AData), PAnsiChar(Buffer), ASize);
{$ELSE}
  raise Exception.Create(sxeUnsupportedEncoding);
{$ENDIF}
{$ENDIF}
...
function sdStringToBool(const AValue: AnsiString): boolean;
var
  Ch: AnsiChar;
begin
...
function sdStringFromBool(ABool: boolean): AnsiString;
const
...
begin
  ...
  begin
    ...
    if Source is TNFDXML then
    begin
      Assign(TNFDXML(Source).FRootNodes);
    end else
      inherited;
end;

procedure TXmlNode.AttributeAdd(const AName, AValue: AnsiString);
var
  Attr: AnsiString;
begin
...
{$IFDEF D4UP}
procedure TXmlNode.AttributeAdd(const AName: AnsiString; AValue: integer);
begin
...
function TXmlNode.AttributeIndexByname(const AName: AnsiString): integer;
...
procedure TXmlNode.ChangeDocument(ADocument: TNFDXML);
var
  i: integer;
begin
  FDocument := ADocument;
...
function TXmlNode.CompareNodeName(const NodeName: AnsiString): integer;
begin
...
constructor TXmlNode.Create(ADocument: TNFDXML);
begin
  inherited Create;
  FDocument := ADocument;
end;

constructor TXmlNode.CreateName(ADocument: TNFDXML;
...
constructor TXmlNode.CreateNameValue(ADocument: TNFDXML; const AName,
  AValue: AnsiString);
begin
  Create(ADocument);
  Name := AName;
...
constructor TXmlNode.CreateType(ADocument: TNFDXML;
...
function TXmlNode.FindNode(const NodeName: AnsiString): TXmlNode;
// Find the first node which has name NodeName. Contrary to the NodeByName
// function, this function will search the whole subnode tree, using the
// DepthFirst method.
var
...
procedure TXmlNode.FindNodes(const NodeName: AnsiString; const AList: TList);
  // local
...
function TXmlNode.FromAnsiString(const s: AnsiString): AnsiString;
begin
  if Utf8Encoded then
...
function TXmlNode.FromWidestring(const W: widestring): AnsiString;
begin
  if Utf8Encoded then
...
function TXmlNode.GetAttributeByName(const AName: AnsiString): AnsiString;
begin
...
function TXmlNode.GetAttributeByNameWide(const AName: AnsiString): widestring;
begin
...
function TXmlNode.GetAttributeName(Index: integer): AnsiString;
begin
  if (Index >= 0) and (Index < AttributeCount) then
    Result := FAttributes.Names[Index];
end;

function TXmlNode.GetAttributePair(Index: integer): AnsiString;
begin
  if (Index >= 0) and (Index < AttributeCount) then
    Result := FAttributes[Index];
end;

function TXmlNode.GetAttributeValue(Index: integer): AnsiString;
var
  P: integer;
  S: AnsiString;
begin
...
function TXmlNode.GetAttributeValueDirect(Index: integer): AnsiString;
var
  P: integer;
  S: AnsiString;
begin
...
function TXmlNode.GetBinaryString: AnsiString;
...
function TXmlNode.GetCascadedName: AnsiString;
...
function TXmlNode.GetFullPath: AnsiString;
...
function TXmlNode.GetIndent: AnsiString;
var
  i: integer;
begin
  if assigned(Document) then
...
function TXmlNode.GetLineFeed: AnsiString;
begin
  if assigned(Document) then
  begin
    case Document.XmlFormat of
...
function TXmlNode.GetValueAsString: AnsiString;
begin
...
function TXmlNode.HasAttribute(const AName: AnsiString): boolean;
var
  i: integer;
begin
  Result := False;
...
function TXmlNode.NodeByAttributeValue(const NodeName, AttribName, AttribValue: AnsiString;
...
function TXmlNode.NodeByName(const AName: AnsiString): TXmlNode;
var
  i: integer;
begin
  Result := nil;
...
function TXmlNode.NodeFindOrCreate(const AName: AnsiString): TXmlNode;
...
function TXmlNode.NodeIndexByName(const AName: AnsiString): integer;
begin
  Result := 0;
...
function TXmlNode.NodeIndexByNameFrom(const AName: AnsiString; AFrom: integer): integer;
begin
  Result := AFrom;
  while Result < NodeCount do
  begin
...
function TXmlNode.NodeNew(const AName: AnsiString): TXmlNode;
// Add a new child node and return its pointer
begin
  Result := Nodes[NodeAdd(TXmlNode.CreateName(Document, AName))];
end;

function TXmlNode.NodeNewAtIndex(Index: integer; const AName: AnsiString): TXmlNode;
// Create a new node with AName, and insert it into the subnode list at location
// Index, and return a pointer to it.
begin
  if (Index >= 0) and (Index <= NodeCount) then
...
procedure TXmlNode.NodesByName(const AName: AnsiString; const AList: TList);
...
procedure TXmlNode.ParseTag(const AValue: AnsiString; TagStart, TagClose: integer);
var
  LItems: TStringList;
begin
...
function TXmlNode.ReadAttributeBool(const AName: AnsiString; ADefault: boolean): boolean;
var
  V: AnsiString;
begin
...
function TXmlNode.ReadAttributeDateTime(const AName: AnsiString;
...
function TXmlNode.ReadAttributeFloat(const AName: AnsiString; ADefault: double): double;
var
  V: AnsiString;
  Code: integer;
begin
...
function TXmlNode.ReadAttributeInteger(const AName: AnsiString; ADefault: integer): integer;
begin
  Result := StrToIntDef(AttributeByName[AName], ADefault);
end;

function TXmlNode.ReadAttributeInt64(const AName: AnsiString; ADefault: int64): int64;
begin
  Result := StrToInt64Def(AttributeByName[AName], ADefault);
end;

function TXmlNode.ReadAttributeString(const AName: AnsiString; const ADefault: AnsiString): AnsiString;
begin
...
function TXmlNode.ReadBool(const AName: AnsiString; ADefault: boolean): boolean;
var
  Index: integer;
begin
  Result := ADefault;
...
{$IFDEF USEGRAPHICS}
procedure TXmlNode.ReadBrush(const AName: AnsiString; ABrush: TBrush);
var
  Child: TXmlNode;
begin
...
function TXmlNode.ReadColor(const AName: AnsiString; ADefault: TColor): TColor;
var
  Index: integer;
begin
  Result := ADefault;
...
function TXmlNode.ReadDateTime(const AName: AnsiString; ADefault: TDateTime): TDateTime;
...
function TXmlNode.ReadFloat(const AName: AnsiString; ADefault: double): double;
var
  Index: integer;
begin
  Result := ADefault;
...
{$IFDEF USEGRAPHICS}
procedure TXmlNode.ReadFont(const AName: AnsiString; AFont: TFont);
var
  Child: TXmlNode;
begin
  Child := NodeByName(AName);
...
procedure TXmlNode.ReadFromString(const AValue: AnsiString);
var
  S: TStream;
begin
...
function TXmlNode.ReadInt64(const AName: AnsiString; ADefault: int64): int64;
var
  Index: integer;
begin
  Result := ADefault;
...
function TXmlNode.ReadInteger(const AName: AnsiString; ADefault: integer): integer;
var
  Index: integer;
begin
  Result := ADefault;
...
{$IFDEF USEGRAPHICS}
procedure TXmlNode.ReadPen(const AName: AnsiString; APen: TPen);
var
  Child: TXmlNode;
begin
  Child := NodeByName(AName);
...
function TXmlNode.ReadString(const AName: AnsiString; const ADefault: AnsiString): AnsiString;
var
  Index: integer;
begin
  Result := ADefault;
...
function TXmlNode.ReadWidestring(const AName: AnsiString; const ADefault: widestring): widestring;
begin
...
// Replace any entity references by the entities, and parse the new content if any
  // local
  function SplitReference(const AValue: AnsiString; var Text1, Text2: AnsiString): AnsiString;
  var
    P: integer;
  begin
    Result := '';
  ...
  // local
  function ReplaceEntityReferenceByNodes(ARoot: TXmlNode; const AValue: AnsiString; var InsertPos: integer; var Text1, Text2: AnsiString): boolean;
  var
...
var
  ...
  Node: TXmlNode;
  V, Reference, Replace, Entity, First, Last: AnsiString;
begin
...
procedure TXmlNode.SetAttributeByName(const AName, Value: AnsiString);
begin
  CheckCreateAttributesList;
  FAttributes.Values[AName] := QuoteString(EscapeString(Value));
end;

procedure TXmlNode.SetAttributeByNameWide(const AName: AnsiString; const Value: widestring);
begin
  SetAttributeByName(AName, FromWidestring(Value));
end;

procedure TXmlNode.SetAttributeName(Index: integer; const Value: AnsiString);
var
  S: AnsiString;
  P: integer;
begin
...
procedure TXmlNode.SetAttributeValue(Index: integer; const Value: AnsiString);
begin
  if (Index >= 0) and (Index < AttributeCount) then
...
procedure TXmlNode.SetBinaryString(const Value: AnsiString);
var
  OldEncoding: TBinaryEncodingType;
begin
  // Set to base64
...
procedure TXmlNode.SetName(const Value: AnsiString);
var
  i: integer;
begin
  if FName <> Value then
...
procedure TXmlNode.SetValueAsString(const AValue: AnsiString);
begin
...
function TXmlNode.ToAnsiString(const s: AnsiString): AnsiString;
begin
  if Utf8Encoded then
...
function TXmlNode.ToWidestring(const s: AnsiString): widestring;
begin
  if Utf8Encoded then
    Result := sdUtf8ToUnicode(s)
  else
...
function TXmlNode.UnescapeString(const AValue: AnsiString): AnsiString;
begin
  if Utf8Encoded then
...
procedure TXmlNode.WriteAttributeBool(const AName: AnsiString; AValue: boolean;
  ADefault: boolean);
var
  Index: integer;
begin
...
procedure TXmlNode.WriteAttributeDateTime(const AName: AnsiString; AValue,
  ADefault: TDateTime);
var
  Index: integer;
begin
...
procedure TXmlNode.WriteAttributeFloat(const AName: AnsiString; AValue, ADefault: double);
var
  Index: integer;
  S: AnsiString;
begin
...
procedure TXmlNode.WriteAttributeInteger(const AName: AnsiString; AValue: integer; ADefault: integer);
var
  Index: integer;
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TXmlNode.WriteAttributeInt64(const AName: AnsiString; const AValue: int64; ADefault: int64);
var
  Index: integer;
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TXmlNode.WriteAttributeString(const AName, AValue, ADefault: AnsiString);
var
  Index: integer;
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TXmlNode.WriteBool(const AName: AnsiString; AValue: boolean; ADefault: boolean);
const
  cBoolValues: array[boolean] of string = ('False', 'True');
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
{$IFDEF USEGRAPHICS}
procedure TXmlNode.WriteBrush(const AName: AnsiString; ABrush: TBrush);
begin
  with NodeFindOrCreate(AName) do
  begin
...
procedure TXmlNode.WriteColor(const AName: AnsiString; AValue, ADefault: TColor);
begin
  if WriteOnDefault or (AValue <> ADefault) then
...
procedure TXmlNode.WriteDateTime(const AName: AnsiString; AValue, ADefault: TDateTime);
// Date MUST always be written in this format:
// YYYY-MM-DD (if just date) or
// YYYY-MM-DDThh:mm:ss.sssZ (if date and time. The Z stands for universal time
// zone. Since Delphi's TDateTime does not give us a clue about the timezone,
...
procedure TXmlNode.WriteFloat(const AName: AnsiString; AValue: double; ADefault: double);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
...
{$IFDEF USEGRAPHICS}
procedure TXmlNode.WriteFont(const AName: AnsiString; AFont: TFont);
begin
  with NodeFindOrCreate(AName) do
  begin
...
procedure TXmlNode.WriteHex(const AName: AnsiString; AValue, Digits: integer; ADefault: integer);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
...
function TXmlNode.WriteInnerTag: AnsiString;
...
procedure TXmlNode.WriteInt64(const AName: AnsiString; AValue, ADefault: int64);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
...
procedure TXmlNode.WriteInteger(const AName: AnsiString; AValue: integer; ADefault: integer);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
      ValueAsString := IntToStr(AValue);
...
{$IFDEF USEGRAPHICS}
procedure TXmlNode.WritePen(const AName: AnsiString; APen: TPen);
begin
  with NodeFindOrCreate(AName) do
  begin
...
procedure TXmlNode.WriteString(const AName, AValue: AnsiString; const ADefault: AnsiString);
begin
  if WriteOnDefault or (AValue <> ADefault) then
    with NodeFindOrCreate(AName) do
      ValueAsString := AValue;
...
function TXmlNode.WriteToString: AnsiString;
var
  S: TsdStringStream;
begin
...
procedure TXmlNode.WriteWidestring(const AName: AnsiString;
...
function TXmlNodeList.ByAttribute(const AName, AValue: AnsiString): TXmlNode;
var
  i: integer;
begin
...
procedure TNFDXML.Assign(Source: TPersistent);
  // local
  procedure SetDocumentRecursively(ANode: TXmlNode; ADocument: TNFDXML);
  var
    i: integer;
  begin
...
begin
  ...
  begin
    ...
    FBinaryEncoding := TNFDXML(Source).FBinaryEncoding;
    FDropCommentsOnParse := TNFDXML(Source).FDropCommentsOnParse;
    FExternalEncoding := TNFDXML(Source).FExternalEncoding;
    FParserWarnings := TNFDXML(Source).FParserWarnings;
    FIndentString := TNFDXML(Source).FIndentString;
    FUseFullNodes := TNFDXML(Source).FUseFullNodes;
    FUtf8Encoded := TNFDXML(Source).FUtf8Encoded;
    FWriteOnDefault := TNFDXML(Source).FWriteOnDefault;
    FXmlFormat := TNFDXML(Source).FXmlFormat;
    FSortAttributes := TNFDXML(Source).FSortAttributes;
    // Assign root
    FRootNodes.Assign(TNFDXML(Source).FRootNodes);
...
procedure TNFDXML.CopyFrom(Source: TNFDXML);
begin
...
constructor TNFDXML.CreateName(const ARootName: AnsiString);
begin
  Create;
  Root.Name := ARootName;
end;

destructor TNFDXML.Destroy;
begin
  FreeAndNil(FRootNodes);
  inherited;
end;

procedure TNFDXML.DoNodeLoaded(Node: TXmlNode);
begin
  if assigned(FOnNodeLoaded) then
    FOnNodeLoaded(Self, Node);
end;

procedure TNFDXML.DoNodeNew(Node: TXmlNode);
begin
  if assigned(FOnNodeNew) then
    FOnNodeNew(Self, Node);
end;

procedure TNFDXML.DoProgress(Size: integer);
begin
  if assigned(FOnProgress) then
    FOnProgress(Self, Size);
end;

procedure TNFDXML.DoUnicodeLoss(Sender: TObject);
begin
  if assigned(FOnUnicodeLoss) then
    FOnUnicodeLoss(Self);
end;

function TNFDXML.GetCommentString: AnsiString;
...
function TNFDXML.GetEncodingString: AnsiString;
begin
  Result := '';
...
function TNFDXML.GetEntityByName(AName: AnsiString): AnsiString;
var
  i, j: integer;
begin
  Result := '';
...
function TNFDXML.GetRoot: TXmlNode;
begin
  Result := FRootNodes.NodeByElementType(xeNormal);
end;

function TNFDXML.GetStyleSheetNode: TXmlNode;
begin
...
function TNFDXML.GetVersionString: AnsiString;
begin
  Result := '';
  if FRootNodes.NodeCount > 0 then
    if FRootNodes[0].ElementType = xeDeclaration then
...
function TNFDXML.IsEmpty: boolean;
var
  R: TXmlNode;
begin
  Result := True;
...
function TNFDXML.LineFeed: AnsiString;
begin
  case XmlFormat of
  xfReadable:
    Result := #13#10;
...
procedure TNFDXML.LoadFromFile(const FileName: AnsiString);
var
  S: TStream;
begin
...
procedure TNFDXML.LoadFromStream(Stream: TStream);
var
...
procedure TNFDXML.ParseDTD(ANode: TXmlNode; S: TStream);
...
procedure TNFDXML.ReadFromStream(S: TStream);
var
  i: integer;
  Node: TXmlNode;
  Enc: AnsiString;
...
procedure TNFDXML.ReadFromString(const AValue: AnsiString);
var
  S: TStream;
begin
  S := TsdStringStream.Create(AValue);
...
procedure TNFDXML.ResolveEntityReferences;
begin
  if assigned(Root) then
    Root.ResolveEntityReferences;
end;

procedure TNFDXML.SaveToFile(const FileName: AnsiString);
var
  S: TStream;
begin
...
procedure TNFDXML.SaveToStream(Stream: TStream);
var
...
procedure TNFDXML.SetCommentString(const Value: AnsiString);
...
procedure TNFDXML.SetEncodingString(const Value: AnsiString);
var
  Node: TXmlNode;
begin
...
procedure TNFDXML.SetVersionString(const Value: AnsiString);
var
  Node: TXmlNode;
begin
...
procedure TNFDXML.WriteToStream(S: TStream);
var
  i: integer;
begin
...
function TNFDXML.WriteToString: AnsiString;
var
  S: TsdStringStream;
begin
  S := TsdStringStream.Create('');
...
procedure TsdStringBuilder.AddString(var S: AnsiString);
var
  {$IFDEF CLR}
  i: integer;
  {$ENDIF}
...
function TsdStringBuilder.StringCopy(AFirst, ALength: integer): AnsiString;
begin
...
function TsdStringBuilder.Value: AnsiString;
begin
